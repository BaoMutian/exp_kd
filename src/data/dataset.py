...
def create_onpolicy_kd_dataset(
    teacher_data_path: Union[str, Path],
    student_data_path: Union[str, Path],
) -> Dataset:
    """
    Create dataset for On-Policy KD (Online Knowledge Distillation).

    On-Policy KD requires prompt-only data for both teacher and student:
    - Teacher prompt: Q+E (with experience) - for scoring student-generated responses
    - Student prompt: Q (no experience) - for generating responses on-the-fly

    Note: No assistant responses are included as they will be generated by the
    ...
    Returns:
        HuggingFace Dataset with 'teacher_prompt_messages', 'student_prompt_messages' columns
    """
    teacher_data, student_data = load_kd_dataset(
        teacher_data_path, student_data_path
    )
    ...
    data = []
    for idx in range(len(teacher_data)):
        teacher_msgs = teacher_data[idx]["messages"]
        student_msgs = student_data[idx]["messages"]

        # Extract prompt-only messages (system + user, no assistant)
        teacher_prompt = [msg for msg in teacher_msgs if msg["role"] != "assistant"]
        student_prompt = [msg for msg in student_msgs if msg["role"] != "assistant"]

        data.append({
            "teacher_prompt_messages": teacher_prompt,
            "student_prompt_messages": student_prompt,
        })
