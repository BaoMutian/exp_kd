...
class OnlineKDDataCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for feature in features:
            ...
            teacher_prompt = self._get_prompt_messages(
                feature["teacher_messages"])
            student_prompt = self._get_prompt_messages(
...
class OnlineKDTrainer(Trainer):
        ...
        **kwargs,
        ...
        self.generation_config = GenerationConfig(
            ...
            temperature=generation_config.get(
                "temperature", 0.7) if generation_config else 0.7,
            top_p=generation_config.get(
                "top_p", 0.8) if generation_config else 0.8,
            top_k=generation_config.get(
                "top_k", 20) if generation_config else 20,
        ...
        logger.info(
            f"OnlineKDTrainer initialized with num_samples={num_samples}")
    ...
    def _prepare_teacher_input_with_response(
        ...
        full_mask = torch.cat(
            [teacher_prompt_mask, student_response_mask], dim=1)
